...
export function useFileUpload(options: FileUploadOptions = {}) {
  ...
  const uploadSingleFile = async (
    ...
    try {
      ...
      // message.success를 직접 사용하지 않고 console.log로 대체 (Naive UI용)
      console.warn('파일이 성공적으로 업로드되었습니다.');
      return uploadedFile;
    } catch (error) {
      console.error('파일 업로드 실패:', error);
      // message.error를 직접 사용하지 않고 console.error로 대체 (Naive UI용)
      console.error(
  ...
  const uploadMultipleFiles = async (
    ...
    if (errors.length > 0) {
      ...
      console.warn(`${errors.length}개 파일 업로드가 실패했습니다.`);
    }

    return results;
  };

  // Naive UI Upload 컴포넌트용 커스텀 업로드 함수
