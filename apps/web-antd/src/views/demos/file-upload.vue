<template>
  <div class="file-upload-demo">
    <!-- 페이지 헤더 -->
    <div class="mb-6">
      <h1 class="text-2xl font-bold mb-2">파일 업로드 데모</h1>
      ...
      <a-tabs v-model:activeKey="uploadTab">
        ...
        <a-tab-pane key="image" tab="이미지 업로드">
          ...
          >
            <div class="upload-area">
              <p class="text-center">
                <PictureOutlined class="text-4xl text-gray-400 mb-2" />
                <br />
        ...
        <a-tab-pane key="document" tab="문서 업로드">
          <FileUploadBasic
            ...
            :allowed-types="['application/pdf', 'text/plain', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']"
            accept=".pdf,.txt,.docx"
            @success="handleUploadSuccess"
            @error="handleUploadError"
          >
    ...
    <!-- 업로드 결과 -->
    <a-card v-if="recentUploads.length > 0" title="3. 최근 업로드된 파일" class="mb-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        >
          ...
          <div class="mt-3 flex gap-2">
            <a-button size="small" @click="copyUrl(file.url)">
              복사
            </a-button>
    ...
    <a-card title="5. 파일 통계" class="mb-6">
      <div v-if="stats" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="text-center p-4 border rounded">
          <div class="text-2xl font-bold text-blue-600">{{ stats.overview.totalFiles }}</div>
          <div class="text-sm text-gray-600">전체 파일</div>
        </div>
        <div class="text-center p-4 border rounded">
          <div class="text-2xl font-bold text-green-600">{{ formatFileSize(stats.overview.totalSize) }}</div>
          <div class="text-sm text-gray-600">전체 크기</div>
        </div>
        <div class="text-center p-4 border rounded">
          <div class="text-2xl font-bold text-purple-600">{{ stats.overview.imageFiles }}</div>
          <div class="text-sm text-gray-600">이미지 파일</div>
        </div>
        <div class="text-center p-4 border rounded">
          <div class="text-2xl font-bold text-orange-600">{{ stats.overview.documentFiles }}</div>
    ...
    <a-card title="6. API 테스트" class="mb-6">
      ...
      <div v-if="testResults.length > 0" class="mt-4">
        <h4>테스트 결과:</h4>
        <div class="bg-gray-100 p-4 rounded mt-2 max-h-60 overflow-y-auto">
...
// Composables
import { useSupabase, useFileUpload, useFileManager } from '#/composables';
import type { UploadedFileInfo, FileStats } from '#/composables';
...
const testAuth = async () => {
  try {
    ...
    if (isSupabaseEnabled.value && isAuthenticated.value) {
      ...
      addTestResult(`인증 성공: ${userInfo.email} (역할: ${userInfo.roles.join(', ')})`);
    } else {
...
const testFileList = async () => {
  try {
    ...
    addTestResult(`파일 목록 조회 성공: ${result.total}개 파일 중 ${result.data.length}개 조회`);
  } catch (error) {
...
const testUpload = async () => {
  try {
    ...
    addTestResult(`업로드 테스트 성공: ${result.originalName} (${formatFileSize(result.fileSize)})`);
  } catch (error) {
